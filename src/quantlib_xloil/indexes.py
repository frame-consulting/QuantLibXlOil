...
def qlIndexFixing(
    index: ql.Index, date: qDate, forecast_todays_fixing=False, trigger=None
) -> float:
...
def qlInterestRateIndexFixingDate(
    index: ql.InterestRateIndex, value_date: qDate, trigger=None
) -> ql.Date:
...
def qlInterestRateIndexCurrency(
...
def qlInterestRateIndexDayCounter(
    index: ql.InterestRateIndex, trigger=None
) -> ql.DayCounter:
...
def qlInterestRateIndexMaturityDate(
    index: ql.InterestRateIndex, value_date: qDate, trigger=None
) -> ql.Date:
...
def qlInterestRateIndexValueDate(
    index: ql.InterestRateIndex, fixing_date: qDate, trigger=None
) -> ql.Date:
...
def qlOvernightIndex(
    ...
    return ql.OvernightIndex(
        family_name, settlement_days, currency, calendar, day_counter, projection_curve
    )
...
def qlEuriborSwapIsdaFixA(
    swap_tenor: qPeriod,
    proj_curve: ql.YieldTermStructureHandle = ql.YieldTermStructureHandle(),
    discount_curve: ql.YieldTermStructureHandle = ql.YieldTermStructureHandle(),
    trigger=None,
) -> ql.SwapIndex:
...
def qlEquityIndex(
    ...
    return ql.EquityIndex(
        name, fixing_calendar, currency, disc_curve, div_curve, qpotprice_qh
    )
