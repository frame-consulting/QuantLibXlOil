...
from .date import qDate, _qDate
from .daycounters import qDayCounter

from .interpolatedyieldcurves import (
...
def qlPiecewiseYieldCurve(
    ...
    #
    return ql.YieldTermStructureHandle(qlPiecewiseYieldCurveAsYts(
        reference_date,
        instruments,
        daycounter,
        traits,
...
def qlPiecewiseYieldCurveWithJumpsAsYts(
    ...
    jump_dates = [ _qDate(d) for d in jump_dates ]
    #
    traits = str(traits).strip().upper()
    if traits not in QL_INTERPOLATION_TRAITS:
        raise ValueError("Invalid traits. Valid values are: " + ", ".join(QL_INTERPOLATION_TRAITS))
    ...
    if curve_class is None:
        raise ValueError(f"Interpolation method not implemented for traits '{traits}' and interpolator '{interpolator}'.")
    yts = curve_class(reference_date, _to_ql_rate_helpers(instruments), daycounter, jumps, jump_dates)
    return yts
...
def qlPiecewiseYieldCurveWithJumps(
    ...
    #
    return ql.YieldTermStructureHandle(qlPiecewiseYieldCurveWithJumpsAsYts(
        reference_date,
        instruments,
        daycounter,
        jumps,
...
def qlPiecewiseSpreadYieldCurve(
    ...
    #
    return ql.YieldTermStructureHandle(qlPiecewiseSpreadYieldCurveAsYts(
        base_curve,
        instruments,
        traits,
        interpolator,
